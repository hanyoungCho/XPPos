...
type
  TConfigForm = class(TForm)
    ...
    GroupBox4: TGroupBox;
    edtClosingAppName: TLabeledEdit;
    btnClosingAppFile: TBitBtn;
    ...
    procedure btnClosingAppFileClick(Sender: TObject);
  private
...
procedure TConfigForm.btnClosingAppFileClick(Sender: TObject);
begin
  with TOpenDialog.Create(nil) do
  try
    InitialDir := Global.HomeDir;
    Filter := 'Executable files(*.exe)|*.exe|Batch script files(*.bat)|*.bat|All files(*.*)|*.*';
    if Execute then
      edtClosingAppName.Text := ExtractFileName(FileName);
  finally
    Free;
  end;
end;
