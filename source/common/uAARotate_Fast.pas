...
//
//  v1.3.0  : 13 Feb 2010
//    Added function GetResizedBitmap
//
//  v1.23 :  3 Apr 2009
...
uses Windows, graphics, Math, SysUtils, Classes;
...
// ** 주의) FastAARotatedBitmap으로 얻어진 TBitmap에 대한 Scanline 메소드는 작동하지 않는다.
//          예를 들어 맨 위 한 라인의 이미지를 불투명 검정색으로 변경하기 위한 아래와 같은 문장은 아무
//          효과가 없다.  (ResultImage : FastAARotatedBitmap으로 얻어진 TBitmap)
//
//           P := ResultImage.Scanline[0];    // P : PARGBArray;
//           for x := 0 to (ResultImage.Width - 1) do
//           begin
//             P[x].rgbRed := 0;
//             P[x].rgbGreen := 0;
//             P[x].rgbBlue := 0;
...
function GetResizedBitmap(SrcBitmap : TBitmap; var DstBitmap : TBitmap; Scale : double) : boolean;
   ...
   begin
     ...
     begin
      ...
      // The sum of (TopL + TopR + BotL + BotR) is always 1
        ...
        begin
         // All the intersected areas are placed within srcbmp region
...
begin
  ...
  //Get scale between (0.1, 10.0)
   ...
   res := dorotate(SrcBitmap.Handle, 0, 0, true, UseAlphaChannel, false, Scale);

   if res <> 0 then
   begin
     try
        tmpBitmap := TBitmap.Create;
        tmpBitmap.Handle := res;

      // ** Assign문으로 이미지를 복사하면 DstBitmap에 대한 Scanline 함수가 작동하지 않는다.
      //    => CopyRect 함수로 대치하였다.
      // DstBitmap.Assign(tmpBitmap);
        DstBitmap.Width := tmpBitmap.Width;
        DstBitmap.Height := tmpBitmap.Height;
        R := Rect(0, 0, tmpBitmap.Width, tmpBitmap.Height);
        DstBitmap.Canvas.CopyRect(R, tmpBitmap.Canvas, R);
        tmpBitmap.Free;
        Result := true;
     except
        exit;
