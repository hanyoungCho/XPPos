...
(*
  ...
  var
    ...
    CRC: integer;
  begin
    Error := GetFileCRC('c:\command.com', CRC, 0, 0);
    if (Error <> 0) then
  ...
  BytesFromStart and BytesBeforEnd enables you get file CRC code for
  any part of a file.
  For example you can get file CRC without first 10 bytes:
...
function GetFileCRC(AFileName: string; var ACRCvalue: longint; ABytesFromStart, ABytesBeforeEnd: longint): integer;
procedure CalcCRC32(var ABuffer: array of byte; ANByte: word; var ACRCvalue: longint);
...
procedure CalcCRC32(var ABuffer: array of byte; ANByte: word; var ACRCvalue: longint);
{
  ...
  1. exclusive-or the input byte with the low-order portion of the CRC register
     to get an INDEX
  2. shift the CRC register eight bits to the right
  3. exclusive-or the CRC register with the contents of CRC_TABLE[INDEX]
  4. repeat steps 1 through 3 for all bytes
}
var
  i: longint;
begin
  for i := 0 to Pred(ANByte) do
    ACRCvalue :=
      (LongWord(ACRCvalue) shr 8) xor CRC_TABLE[ABuffer[i] xor (ACRCvalue and $000000FF)];
end;
...
begin
  ...
  try
    ...
    TotalBytes := 0;
    FSize := (FileSize(F) - ABytesFromStart - ABytesBeforeEnd);
    Seek(F, ABytesFromStart); //Bypass file header
    MustRead:= SizeOf(Buffer);

    repeat
      if (TotalBytes + MustRead > FSize) then
        MustRead := (FSize - TotalBytes);
      {$i-}
      BlockRead(F, Buffer, MustRead, BytesRead);
      {$i+}
      Error := IOResult;
      if (Error <> 0) then
      begin
      ...
      CalcCRC32(Buffer, BytesRead, ACRCvalue);
      Inc(TotalBytes, BytesRead)
    until (TotalBytes >= FSize);

    ACRCvalue := ABS(not ACRCvalue);
    Result := 0; //Successed
  finally
    CloseFile(F);
  end;
